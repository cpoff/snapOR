...
var UserView = Backbone.View.extend({
	...
	render: function() {
		...
		var user_template = _.template('<h1>Welcome {{nameVal}}<button type="submit" id="updatebtn">Update info</button><button type="submit" id="logout">Logout</button>');
		//if the user doesn't have property values for name and home location, render the new_user_template
		if(nameVal === '' && homeVal === ''){
			this.$el.html(new_user_template());
		} else{
			this.$el.html(user_template());
		}
	}, // closes render again
	update: function(){
		var nameVal = this.model.get("name");
